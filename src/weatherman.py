...
class Weatherman:
    ...
    def read_forecast(self):
        try:
            ...
            return None
        print("Success: Open-Meteo forecasts read from file!")
...
def main():
    ...
    weatherman.prompt = f"""
        You are an enthusastic local meterologist named Sunny McSkies.
        ...
        Please write a 8-12 sentence weather report as if you were making a short-form video for your local station's social media account.
        - Use a warm, light-hearted tone
        - Mention today's high/low and overall feel
        - Call out any chance of precipitation and when it is
        ...
        Don't mention days of the week like Monday, Tuesday, etc.
    """
...
if __name__ == ('__main__'):
    ...
    load_dotenv()
    LOCATION = "RoundRock"
    NTFY_TOPIC = "sunny-roundrock"
    weatherman = Weatherman(location_override=LOCATION)
    weatherman.read_forecast()
    weatherman.truncate_hourly(hourly_list=weatherman.hourly_forecast['forecast'], max_hours=24)
    weatherman.days = len(weatherman.daily_forecast['forecast'])
    weatherman.hours = len(weatherman.truncated_hourly_forecast)
    ...
    # This loop will retry the query until it gets a long enough report
    # Sometimes reports can be short, not useful
    result = 0
    while result < 20:
        weatherman.query_model()
        result = len(weatherman.weather_report)
    weatherman.write_report()
    weatherman.send_ntfy(topic=NTFY_TOPIC)
